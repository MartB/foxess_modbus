...
class ModbusBatterySensor(ModbusSensor):
    ...
    def native_value(self) -> Any:
        # Some models (e.g. the H3 Pro) have no BMS connect state register at all. Don't treat that as
        # "disconnected", or the sensor is permanently unavailable
        if self._bms_connect_state_address is None:
            return super().native_value

        # 0: Initial state, 1: OK, 2: NG
        value = self._controller.read(self._bms_connect_state_address, signed=False)
        if value is None or (value & 0xFF) != 1:
            return None
